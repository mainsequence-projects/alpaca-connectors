...
class AlpacaBarsSupportTests(unittest.TestCase):
    ...
    def test_normalize_stock_bars_frame_filters_duplicates_and_last_updates(self) -> None:
        ...
        self.assertEqual(
            normalized.iloc[0]["open_time"],
            pd.Timestamp("2026-04-11T13:30:00Z"),
        )
        self.assertEqual(float(normalized.iloc[0]["close"]), 101.6)

    def test_resolve_asset_bindings_reuses_stored_symbol_for_canonical_alpaca_asset(
        self,
    ) -> None:
        class StubAsset:
            def __init__(self):
        ...
        self.assertEqual(bindings[0].alpaca_symbol, "FI")
        self.assertEqual(
            bindings[0].unique_identifier,
            "ALPACA::11111111-1111-4111-8111-111111111111",
        )

    def test_resolve_asset_bindings_does_not_use_figi_for_non_alpaca_identity(self) -> None:
        class StubAsset:
            unique_identifier = "PUBLIC::FI"
            figi = "BBG000BJKPG0"
            ticker = "FI"
