...
description: Create and validate backend-managed API 2.1.0 deployment declarations under .mainsequence/workflows, including target-owned environment variables, FastAPI browser origins, and authenticated-repository-action-authorized Static Site navigation placement.
---
...
A `2.1.0` Static Site declaration may propose a managed navigation link:
...
Declare every widget implementation, including first-party widgets, through the ordinary
`resource_release` workflow kind:

```yaml
- key: widget-extensions
  kind: resource_release
  spec:
    release_kind: widget_extension
    name: widget-extensions
    root_directory: widget-extensions
```
